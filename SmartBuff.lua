...
-- for watching
SMARTBUFF_CGroups = cGroups
SMARTBUFF_cBuffs = cBuffs
...
function SMARTBUFF_OnEvent(self, event, arg1, arg2, arg3, arg4, arg5)
  ...
  if (event == "UNIT_AURA") then
    ...
    if ((sPlayerClass == "HUNTER" or sPlayerClass == "HERO") and O.AntiDaze and (arg1 == "player" or string.find(arg1, "^party") or string.find(arg1, "^raid") or string.find(arg1, "pet"))) then
  ...
  --[[  
  elseif (event == "CHAT_MSG_ADDON" and O.CTRASync) then
...
function SMARTBUFF_AddMsgD(msg, r, g, b)
  ...
  if (DebugChatFrame and SMARTBUFF_Options and O.Debug) then
...
function DebugMessage(msg, ...)
  if (SMARTBUFF_Options and O.Debug) then
    if ViragDevTool_AddData then 
        ViragDevTool_AddData({...}, "SmartBuff: "..msg);
    end
  end
end

function DebugMessageBreak()
  if (SMARTBUFF_Options and O.Debug) then
    if ViragDevTool_AddData then 
...
function SMARTBUFF_SetUnits()
  ...
  elseif (iGroupSetup == 2) then
    ...
    if (sPlayerClass == "HUNTER" or sPlayerClass == "WARLOCK" or sPlayerClass == "DEATHKNIGHT" or sPlayerClass == "MAGE" or sPlayerClass == "HERO") then
      cGroups[1][9] = "pet";
    end
    for j = 1, 4, 1 do
      cGroups[1][j] = "party"..j;
  ...
  -- Solo Setup
	...
	DebugMessage("Solo Unit-Setup finished", cGroups)
  end
...
function SMARTBUFF_AddSoloSetup()
  ...
  DebugMessage("SMARTBUFF_AddSoloSetup -> sPlayerClass", sPlayerClass)
  DebugMessage("SMARTBUFF_AddSoloSetup -> test", sPlayerClass == "HUNTER" or sPlayerClass == "WARLOCK" or sPlayerClass == "DEATHKNIGHT" or sPlayerClass == "MAGE" or sPlayerClass == "HERO")
  if (sPlayerClass == "HUNTER" or sPlayerClass == "WARLOCK" or sPlayerClass == "DEATHKNIGHT" or sPlayerClass == "MAGE" or sPlayerClass == "HERO") then
	cGroups[0][1] = "pet";
	DebugMessage("Adding a pet to the buff list", cGroups)
  end
...
-- Get the spellbook ID of a specific rank of a known spell, by name.
-- Needed because WoW spell IDs for different ranks of the same spell are NOT sequential
-- (e.g. Blessing of Wisdom: Rank 1 = 19742, Rank 2 = 19850, Rank 3 = 19852, ...),
-- so the rank cannot be derived by simple arithmetic on the spell ID.
function SMARTBUFF_GetSpellIDByRank(spellname, rank)
  if (spellname == nil or rank == nil) then
    return nil;
  end

  spellname = string.lower(spellname);
  local targetRank = "Rank " .. rank;

  local i = 0;
  local spellN, spellR;
  while true do
    i = i + 1;
    spellN, spellR = GetSpellName(i, SMARTBUFF_BOOK_TYPE_SPELL);
    if (not spellN) then
      break;
    end
    if (string.lower(spellN) == spellname and spellR == targetRank) then
      return i;
    end
  end
...
-- END SMARTBUFF_GetSpellIDByRank


-- Set the buff array
function SMARTBUFF_SetBuffs()
  --if (not O.Toggle) then return; end
...
function SMARTBUFF_SetBuff(buff, i)
  ...
  if (B[CS()][ct][name] == nil) then
    B[CS()][ct][name] = { };
    B[CS()][ct][name].EnableS = false;
    B[CS()][ct][name].EnableG = false;
    B[CS()][ct][name].SelfOnly = false;
    B[CS()][ct][name].SelfNot = false;
    B[CS()][ct][name].CIn = false;
    B[CS()][ct][name].COut = true;
    B[CS()][ct][name].MH = false;
    B[CS()][ct][name].OH = false;
    B[CS()][ct][name].Reminder = true;
    B[CS()][ct][name].RBTime = 0;
    B[CS()][ct][name].ManaLimit = 0;
    ...
    if (cBuffs[i].Type == SMARTBUFF_CONST_GROUP) then
      for n in ipairs(cClasses) do
        if (not string.find(cBuffs[i].Exclude, cClasses[n])) then
          B[CS()][ct][name][cClasses[n]] = true;
        else
          B[CS()][ct][name][cClasses[n]] = false;
        end
      end
    end
  end  
  ...
  -- update to 1.12b
  if (B[CS()][ct][name].ManaLimit == nil) then
    B[CS()][ct][name].ManaLimit = 0;
  end
  
  -- update to 2.0i
  if (B[CS()][ct][name].SelfNot == nil) then
    B[CS()][ct][name].SelfNot = false;
  end
  
  -- update to 2.1a
  if (B[CS()][ct][name].AddList == nil) then
    B[CS()][ct][name].AddList = { };
  end  
  if (B[CS()][ct][name].IgnoreList == nil) then
    B[CS()][ct][name].IgnoreList = { };
  end  
      ...
      for _, unit in pairs(cGrp[subgroup]) do
        ...
        if (unit and UnitExists(unit) and UnitIsConnected(unit) and UnitIsFriend("player", unit) and UnitIsPlayer(unit) and not UnitIsDeadOrGhost(unit)) then
          ...
          while (cBuffs[i] and cBuffs[i].BuffS) do
            ...
            if (cBuffs[i].BuffG and B[CS()][ct][buffS].EnableG and cBuffs[i].IDG ~= nil and cBuffs[i].DurationG > 0
              and (sPlayerClass ~= "PALADIN" or not cClassGroups or (sPlayerClass == "PALADIN" and (B[CS()][ct][buffS][subgroup] or (type(subgroup) == "number" and subgroup == 0))))) then
          ...
          if (cBuffs[i].BuffG and B[CS()] and B[CS()][ct] and B[CS()][ct][cBuffs[i].BuffS].EnableG and cBuffs[i].IDG ~= nil
            ...
            and not SmartBuff_Blessing_GroupHasOverride(cBuffs[i].BuffS, units)) then
                ...
                if (rbTime > 0 and rbTime >= btl) then
                  ...
                  if (mode == 1) then
                    ...
                    if (btl < 0) then
                      ...
                      tLastCheck = GetTime() - O.AutoTimer + 0.5;
                      return;
                    end
                  end
                end
              ...
              for _, unit in pairs(unitsGrp) do
                ...
                --if (unit and (UnitIsPlayer(unit) or (sPlayerClass == "PALADIN" and (UnitPlayerOrPetInParty(unit) or UnitPlayerOrPetInRaid(unit)))) and not SMARTBUFF_IsInList(unit, UnitName(unit), B[CS()][ct][buffnS].IgnoreList)) then
                if (unit and UnitIsPlayer(unit) and not SMARTBUFF_IsInList(unit, UnitName(unit), B[CS()][ct][buffnS].IgnoreList)) then

                  SMARTBUFF_AddMsgD("Checking2 " .. buffnG .. " " .. unit);
                  n = n + 1;
                  --if (UnitExists(unit) and not UnitIsDeadOrGhost(unit) and not UnitIsCorpse(unit) and UnitIsConnected(unit) and UnitIsVisible(unit) and not UnitOnTaxi(unit) and (SMARTBUFF_IsPlayer(unit) or not O.AdvGrpBuffRange or not SpellHasRange(cBuffs[i].BuffG) or (IsSpellInRange(cBuffs[i].BuffG, unit) == 1))) then
              ...
              if (mode == 1 and m >= B[CS()][ct].GrpBuffSize and n >= B[CS()][ct].GrpBuffSize) then
              ...
              if (unitL ~= nil and unitU ~=nil and unitB ~= nil and cBuffs[i].IDG ~= nil) then
                ...
                if (idL ~= nil and idU ~= nil and idL == idU and rank > 0 and m >= B[CS()][ct].GrpBuffSize and n >= B[CS()][ct].GrpBuffSize) then
                  ...
                  if (reagent and mode ~= 1) then
                    ...
                    if (rc > 0) then
                      ...
                      B[CS()][ct][cBuffs[i].BuffS].EnableG = false;
                    end
...
function SMARTBUFF_BuffUnit(unit, subgroup, mode, spell)
  ...
  if (UnitExists(unit) and UnitIsFriend("player", unit) and not UnitIsDeadOrGhost(unit) and not UnitIsCorpse(unit)
    ...
    while (cBuffs[i] and cBuffs[i].BuffS) do
	  ...
	  if (not bs) then bUsable = false end
      ...
      if (bUsable and bs.SelfNot and SMARTBUFF_IsPlayer(unit)) then
        bUsable = false;
      end
      
      if (bUsable and UnitMana("player") < bs.ManaLimit) then
        bUsable = false;
        --SMARTBUFF_AddMsgD(cBuffs[i].BuffS .. " is below the powertype threshold!");
      end
      
	  -- Check for buffs which depends on a pet
	  if (bUsable and sPlayerClass == "HERO" and buffnS == SMARTBUFF_SOULLINK and not UnitExists("pet")) then
        bUsable = false;
      elseif (bUsable and sPlayerClass == "WARLOCK" and buffnS == SMARTBUFF_SOULLINK and not UnitExists("pet")) then
        bUsable = false;
      end
      ...
      if (bUsable and bs.EnableS and (cBuffs[i].IDS ~= nil or SMARTBUFF_IsItem(cBuffs[i].Type))
        ...
        if (not bs.SelfOnly or (bs.SelfOnly and SMARTBUFF_IsPlayer(unit))) then
          ...
          if (cd <= 0 or (mode == 1 and cd <= 1.5)) then
			...
			local isInIgnoreList = SMARTBUFF_IsInList(unit, un, bs.IgnoreList);
			local isInAddList = SMARTBUFF_IsInList(unit, un, bs.AddList);
			local buffTypeIsGroup = cBuffs[i].Type == SMARTBUFF_CONST_GROUP;
			local buffTypeIsNotGroup = cBuffs[i].Type ~= SMARTBUFF_CONST_GROUP;
			local unitCreatureTypeIsHumanoid = uct == SMARTBUFF_HUMANOID;
			...
			DebugMessageBreak();
			DebugMessage("Checking " .. un);
			DebugMessage("isInAddList:", isInAddList);
			DebugMessage("isInIgnoreList:", isInIgnoreList);
			DebugMessage("buffTypeIsGroup:", buffTypeIsGroup);
			DebugMessage("buffTypeIsNotGroup:", buffTypeIsNotGroup);
			DebugMessage("uct:", uct);
			DebugMessage("unitCreatureTypeIsHumanoid:", unitCreatureTypeIsHumanoid);
			DebugMessage("unitCreatureTypeIsBeast:", unitCreatureTypeIsBeast);
			DebugMessage("unitCreatureTypeIsElemental:", unitCreatureTypeIsElemental);
			DebugMessage("unitCreatureTypeIsDemon:", unitCreatureTypeIsDemon);
			DebugMessage("unitCreatureTypeIsUndead:", unitCreatureTypeIsUndead);
			DebugMessage("unitClassIsDruid:", unitClassIsDruid);
			DebugMessage("somethingBuffs:", somethingBuffs);
			DebugMessage("hpet:", hpet);
			DebugMessage("dkpet:", dkpet);
			DebugMessage("wpet:", wpet);
			DebugMessageBreak();

			--local test = not isInIgnoreList and ((buffTypeIsGroup and ((somethingBuffs and (unitCreatureTypeIsHumanoid or (unitClassIsDruid and (unitCreatureTypeIsBeast or unitCreatureTypeIsElemental))))
   --           or (hpet and unitCreatureTypeIsBeast and uc ~= "DRUID")
   --           or (dkpet and unitCreatureTypeIsUndead)
   --           or (wpet and (unitCreatureTypeIsDemon or (uc ~= "DRUID" and unitCreatureTypeIsElemental)) and ucf ~= SMARTBUFF_DEMONTYPE)))
			--  or (buffTypeIsNotGroup and SMARTBUFF_IsPlayer(unit))
   --           or isInAddList);

			local test = not isInIgnoreList and ((buffTypeIsGroup and ((somethingBuffs and unitCreatureTypeIsHumanoid)
              or (hpet and unitCreatureTypeIsBeast)
              or (dkpet and unitCreatureTypeIsUndead)
            ...
            if (test) then
                  ...
                  if (offH and SMARTBUFF_CanApplyWeaponBuff(buffnS, 17)) then
                    if (bOh) then
                      if (rbTime > 0 and cBuffs[i].DurationS >= 1) then
                        ...
                        if (tOh <= rbTime or (O.CheckCharges and cBuffs[i].CanCharge and charges > 0 and charges <= O.MinCharges)) then
                          buff = buffnS;
                          bt = tOh;
                          bExpireOh = true;                          
                        end                      
              ...
              -- Normal buff ------------------------------------------------------------------------
                ...
                if (unit ~= "target" and buff == nil and cBuffs[i].DurationS >= 1 and rbTime > 0) then
                  ...
                  if ((bt and bt <= rbTime) or (O.CheckCharges and cBuffs[i].CanCharge and charges > 0 and charges <= O.MinCharges)) then
                    if (buffname) then
                      buff = buffname;
                    else
                      buff = buffnS;
...
function SMARTBUFF_SetMissingBuffMessage(target, buff, bCanCharge, nCharges, tBuffTimeLeft, bExpire, bExpireOh)
  ...
  if (O.ToggleAutoSplash and not SmartBuffOptionsFrame:IsVisible()) then
    ...
    if (O.CheckCharges and bCanCharge and nCharges > 0 and nCharges <= O.MinCharges and (bExpire or bExpireOh)) then
...
function SMARTBUFF_doCast(unit, id, spellName, levels, type)
  ...
  --if (type == SMARTBUFF_CONST_GROUP and (O.AdvGrpBuffRange and SpellHasRange(spellName) and IsSpellInRange(spellName, unit) ~= 1)) then  
...
function SMARTBUFF_CheckUnitLevel(unit, spellId, spellLevels)
  ...
  if (i > 0) then
    if (i ~= spellRank) then
      -- need a lower rank than the one we started with - look it up by name/rank,
      -- since spell IDs across ranks are not sequential and cannot be computed by subtraction
      Id = SMARTBUFF_GetSpellIDByRank(spellName, i);
    end
    if (Id and spellName) then
...
function SMARTBUFF_Options_Init(self)
  ...
  if (O.AdvGrpBuffCheck == nil) then  O.AdvGrpBuffCheck = false; end
  if (O.AdvGrpBuffRange == nil) then  O.AdvGrpBuffRange = false; end
  ...
  if (O.FirstStart ~= SMARTBUFF_VERSION) then
    O.FirstStart = SMARTBUFF_VERSION;
...
function SMARTBUFF_OAdvGrpBuffCheck()
  O.AdvGrpBuffCheck = not O.AdvGrpBuffCheck;
end
function SMARTBUFF_OAdvGrpBuffRange()
  O.AdvGrpBuffRange = not O.AdvGrpBuffRange;
end
...
function SMARTBUFF_OToggleBuff(s, i)
  ...
  if (s == "S") then
    B[CS()][currentTemplate][name].EnableS = not B[CS()][currentTemplate][name].EnableS;
    if (B[CS()][currentTemplate][name].EnableS) then
  ...
  elseif (s == "G") then
    B[CS()][currentTemplate][name].EnableG = not B[CS()][currentTemplate][name].EnableG;
  end
...
function SmartBuff_BuffSetup_Show(i)
  ...
  if(SmartBuff_BuffSetup:IsVisible() and i == iLastBuffSetup) then
    ...
    if (icon2 and B[CS()][ct][name].EnableG) then
    ...
    SmartBuff_BuffSetup_cbSelf:SetChecked(B[CS()][ct][name].SelfOnly);
    SmartBuff_BuffSetup_cbSelfNot:SetChecked(B[CS()][ct][name].SelfNot);
    SmartBuff_BuffSetup_cbCombatIn:SetChecked(B[CS()][ct][name].CIn);
    SmartBuff_BuffSetup_cbCombatOut:SetChecked(B[CS()][ct][name].COut);
    SmartBuff_BuffSetup_cbMH:SetChecked(B[CS()][ct][name].MH);
    SmartBuff_BuffSetup_cbOH:SetChecked(B[CS()][ct][name].OH);
    SmartBuff_BuffSetup_cbReminder:SetChecked(B[CS()][ct][name].Reminder);
    SmartBuff_BuffSetup_txtManaLimit:SetNumber(B[CS()][ct][name].ManaLimit);
       
    --SMARTBUFF_AddMsgD("Test Buff setup show 1");
    SmartBuff_BuffSetup_RBTime:SetValue(B[CS()][ct][name].RBTime);
    if (cBuffs[i].DurationS > 0) then
      ...
      getglobal(SmartBuff_BuffSetup_RBTime:GetName().."Text"):SetText(B[CS()][ct][name].RBTime .. "\nsec");
    ...
    for _ in pairs(cClasses) do
      ...
      if (hidden) then 
        ...
        cb:SetChecked(B[CS()][ct][name][cClasses[n]]);
        cb:Show();
        btn:Show();
      end
    end
...
function SmartBuff_BuffSetup_OnClick()
  ...
  B[CS()][ct][name].SelfOnly = SmartBuff_BuffSetup_cbSelf:GetChecked();
  B[CS()][ct][name].SelfNot = SmartBuff_BuffSetup_cbSelfNot:GetChecked();
  B[CS()][ct][name].CIn  = SmartBuff_BuffSetup_cbCombatIn:GetChecked();
  B[CS()][ct][name].COut = SmartBuff_BuffSetup_cbCombatOut:GetChecked();
  B[CS()][ct][name].MH = SmartBuff_BuffSetup_cbMH:GetChecked();
  B[CS()][ct][name].OH = SmartBuff_BuffSetup_cbOH:GetChecked();
  B[CS()][ct][name].Reminder = SmartBuff_BuffSetup_cbReminder:GetChecked();
  
  B[CS()][ct][name].RBTime = SmartBuff_BuffSetup_RBTime:GetValue();
  getglobal(SmartBuff_BuffSetup_RBTime:GetName().."Text"):SetText(B[CS()][ct][name].RBTime .. "\nsec");
  ...
  if (cBuffs[i].Type == SMARTBUFF_CONST_GROUP) then
    ...
    for _ in pairs(cClasses) do
      n = n + 1;
      cb = getglobal("SmartBuff_BuffSetup_cbClass"..n);
      B[CS()][ct][name][cClasses[n]] = cb:GetChecked();
    end
  end
...
function SMARTBUFF_Options_OnShow()
  ...
  getglobal(SmartBuffOptionsFrameAutoTimer:GetName().."Text"):SetText(SMARTBUFF_OFT_AUTOTIMER.." "..O.AutoTimer.." sec");
  ...
  SmartBuffOptionsFrame_cbAdvancedGrpBuffCheck:SetChecked(O.AdvGrpBuffCheck);
  SmartBuffOptionsFrame_cbAdvancedGrpBuffRange:SetChecked(O.AdvGrpBuffRange);
  ...
  getglobal(SmartBuffOptionsFrameRebuffTimer:GetName().."Text"):SetText(SMARTBUFF_OFT_REBUFFTIMER.." "..O.RebuffTimer.." sec");

  SmartBuffOptionsFrameBLDuration:SetValue(O.BlacklistTimer);
  getglobal(SmartBuffOptionsFrameBLDuration:GetName().."Text"):SetText(SMARTBUFF_OFT_BLDURATION.." "..O.BlacklistTimer.." sec");
...
function SmartBuffOptionsFrameAutoTimer_OnValueChanged(self)
  ...
  getglobal(self:GetName().."Text"):SetText(SMARTBUFF_OFT_AUTOTIMER.." "..O.AutoTimer.." sec");
end
...
function SmartBuffOptionsFrameGrpBuffSize_OnValueChanged(self)
  ...
  getglobal(self:GetName().."Text"):SetText(s.." "..B[CS()][ct].GrpBuffSize);
end

function SmartBuffOptionsFrameRebuffTimer_OnValueChanged(self)
  O.RebuffTimer = self:GetValue();
  getglobal(self:GetName().."Text"):SetText(SMARTBUFF_OFT_REBUFFTIMER.." "..O.RebuffTimer.." sec");
end
...
function SmartBuffOptionsFrameBLDuration_OnValueChanged(self)
  ...
  getglobal(self:GetName().."Text"):SetText(SMARTBUFF_OFT_BLDURATION.." "..O.BlacklistTimer.." sec");
end
...
function SMARTBUFF_SetCheckButtonBuffs(mode) 
  ...
  getglobal(SmartBuffOptionsFrameGrpBuffSize:GetName().."Text"):SetText(s.." "..B[CS()][ct].GrpBuffSize);
  ...
  while (i <= maxCheckButtons) do
    ...
    if (cBuffs[i] and (cBuffs[i].IDS ~= nil or SMARTBUFF_IsItem(cBuffs[i].Type))) then
      if (cBuffs[i].IDG ~= nil and objG ~= nil) then
        ...
        objG:SetChecked(B[CS()][currentTemplate][cBuffs[i].BuffS].EnableG);
        objG:Show();
      else
      ...
      objS:SetChecked(B[CS()][currentTemplate][cBuffs[i].BuffS].EnableS);
      objS:Show();
    else
